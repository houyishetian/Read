...
//  </tr>
                ...
                if (bookInfo != null) {
                    ...
                    List<String> resolveBookNameResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<td class=\"odd\"><a href=\"([^\"^\n]{1,})\">([^\"^\n]{1,})</a></td>", Arrays.asList(1,2),true);
                    if (resolveBookNameResult != null && resolveBookNameResult.size() != 0) {
                        bookInfo.setBookName(resolveBookNameResult.get(1));
                    }

                    List<String> resolveLastChapterResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<td class=\"even\"><a href=\"([^\"^\n]{1,})\" target=\"_blank\">([^\"^\n]{1,})</a></td>", Arrays.asList(1,2),true);
                    ...
                    List<String> resolveLastUpdateResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<td class=\"odd\" align=\"center\">([^\"^\n]{1,})</td>", Arrays.asList(1),true);
                    if (resolveLastUpdateResult != null && resolveLastUpdateResult.size() != 0) {
                        bookInfo.setLastUpdate(resolveLastUpdateResult.get(0));
                    }

                    List<String> resolveAuthorResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<td class=\"odd\">([^\"^\n]{1,})</td>", Arrays.asList(1),true);
            ...
            while ((current = reader.readLine()) != null) {
                ...
                List<String> resolveLastUpdateResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<th>最后更新</th><td>&nbsp;([^\n^<]{1,})</td>", Arrays.asList(1));
                if (resolveLastUpdateResult != null && resolveLastUpdateResult.size() != 0) {
                    bookInfo.setLastUpdate(resolveLastUpdateResult.get(0));
                    continue;
                }
                ...
                List<String> resolveLastChapterResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "最新章节：<a href=\"([^\n^\"]{1,})\">([^\\n]{1,})</a></p>", Arrays.asList(1,2));
                if (resolveLastChapterResult != null && resolveLastChapterResult.size() != 0) {
                    bookInfo.setBookLink(resolveLastChapterResult.get(0));
                    bookInfo.setLastChapter(resolveLastChapterResult.get(1));
                    continue;
                ...
                List<String> resolveAuthorNameResult = StringKtUtil.Companion.getDataFromContentByRegex(current, "<th>文章作者</th><td>&nbsp;([^\n^<]{1,})</td>", Arrays.asList(1));
